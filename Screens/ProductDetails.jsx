...
const ProductDetails = ({navigation, route}) => {
  ...
  // }, 0);
  const productQuantity = cart.reduce((total, item) => {
    if (item.productId === product.productId && item.type !== 'antigaspi') {
      return total + item.qty;
    }
    return total;
  }, 0);
  ...
  const incrementhandler = async () => {
    ...
    try {
      const stockAvailable = await checkStockForSingleProduct(
        product.productId,
      );
        ...
        <View style={{flexDirection: 'column', gap: 20}}>
          <View style={{flexDirection: 'row'}}>
            ...
            <View style={styles.qtyContainer}>
              ...
              <TouchableOpacity
                onPress={() =>
                  incrementhandler(product.productId, product.offre)
                }
                style={{
